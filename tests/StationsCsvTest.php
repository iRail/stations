...
class StationsCsvTest extends TestCase
{
    /**
    ...
    public function testAll()
    {
        ...
        $result = StationsCsv::getStations();
    ...
    public function testBrussels()
    {
        ...
        $result1 = StationsCsv::getStations('Brussel');
        $result2 = StationsCsv::getStations('Brussels');
        $result3 = StationsCsv::getStations('Bruxelles');
        $result4 = StationsCsv::getStations('Bru.-Noord / Brux.-Nord');
        $result5 = StationsCsv::getStations('Brussels Airport - Zaventem');
        //https://github.com/iRail/stations/issues/137
        $result6 = StationsCsv::getStations('Brux.-Midi');
    ...
    public function testExactNameQueries()
    {
        ...
        $stations = StationsCsv::getStations();
        echo("Testing exact name queries...");
        foreach ($stations as $station) {
            $result = StationsCsv::getStations($station->getName());
            ...
            foreach ($station->getLocalizedNames() as $alternative) {
                $result = StationsCsv::getStations($alternative);
        ...
        $result = StationsCsv::getStations("hal");
        $this->assertGreaterThan(1, count($result));

        $result = StationsCsv::getStations("halle");
        $this->assertGreaterThan(1, count($result));

        $result = StationsCsv::getStations("asse");
        $this->assertGreaterThan(1, count($result));

        $result = StationsCsv::getStations("heist");
        $this->assertGreaterThan(1, count($result));

        $result = StationsCsv::getStations("mechelen");
        $this->assertGreaterThan(1, count($result));

        $result = StationsCsv::getStations("mortsel");
        $this->assertGreaterThan(1, count($result));
    }
    ...
    public function testSpacesAndDashes()
    {
        ...
        foreach ($names as $name) {
            $station = StationsCsv::getStations($name);
    ...
    {
        //Launch a query for Brussels in various ways
        $result1a = StationsCsv::getStations('Ville-Pommerœul');
        $result1b = StationsCsv::getStations('Ville-Pommeroeul');
    ...
    public function testEdgeCases()
    {
        ...
        $result1 = StationsCsv::getStations('Brussel Nat (be)');
        $this->assertCount(1, $result1);

        //test whether sint st. and saint return the same result
        $result2a = StationsCsv::getStations('st pancras');
        $result2b = StationsCsv::getStations('saint pancras');
        $result2c = StationsCsv::getStations('st-pancras');
        $result2d = StationsCsv::getStations('st.-pancras');
        ...
        $result3a = StationsCsv::getStations('frankfurt am main');
        $result3b = StationsCsv::getStations('frankfurt main');
        $this->assertEquals($result3a[0]->getUri(), $result3b[0]->getUri());

        // Check whether flughhaven am main don't get mixed up and all work
        $result4a = StationsCsv::getStations('frankfurt am main flughafen');
        $result4b = StationsCsv::getStations('frankfurt flughafen');
        $result4c = StationsCsv::getStations('frankfurt main flughafen');
        $this->assertEquals($result4a[0]->getUri(), $result4b[0]->getUri());
        $this->assertEquals($result4b[0]->getUri(), $result4c[0]->getUri());

        $result5a = StationsCsv::getStations("braine l'alleud");
        $result5b = StationsCsv::getStations('braine l alleud'); //for hafas purposes: https://github.com/iRail/hyperRail/issues/129
        ...
        $result6a = StationsCsv::getStations('La Louviere- Centre');
        $result6b = StationsCsv::getStations('La Louvière-Centre');
        $this->assertEquals($result6a[0]->getUri(), $result6b[0]->getUri());

        $result7a = StationsCsv::getStations('Vivier D Oie');
        $result7b = StationsCsv::getStations('Vivier D Oie / Diesdelle');
        $result7c = StationsCsv::getStations('Diesdelle/Vivier d\'Oie');
    ...
    public function testId()
    {
        ...
        $result1 = StationsCsv::getStationFromID('008892007');
        $result2 = StationsCsv::getStationFromID('BE.NMBS.008892007');
        $result3 = StationsCsv::getStationFromID('http://irail.be/stations/NMBS/008892007');
        $noResult = StationsCsv::getStationFromID('000000000');
    ...
    {
        //test whether the right object is returned
        $result1 = StationsCsv::getStationFromID('008821535');
        $result2 = StationsCsv::getStations('Kapellen')[0];

        $result3 = StationsCsv::getStationFromID('008200518');
        $result4 = StationsCsv::getStations('Capellen')[0];
    ...
    public function testSort()
    {
        ...
        $results = StationsCsv::getStations('Gent', 'be', true);
        $result1 = $results[0];
        $ghentsp = StationsCsv::getStationFromID('http://irail.be/stations/NMBS/008892007');

        $this->assertEquals($ghentsp->getName(), $result1->getName());

        $results = StationsCsv::getStations('Brussel', '', true);
        $result2 = $results[0];
        //The busiest station in Brussels is the south one
        $brusselssouth = StationsCsv::getStations('Brussels South')[0];
    ...
    {
        $dusseldorfFlughafen = StationsCsv::getStationFromID('http://irail.be/stations/NMBS/008039904');

        $results = StationsCsv::getStations('Düsseldorf Flughafen Hbf', 'de', true);
        ...
        $results = StationsCsv::getStations('Dusseldorf Flughafen Hbf', 'de', true);
        $this->assertGreaterThanOrEqual(1, count($results));
        $result1 = $results[0];
